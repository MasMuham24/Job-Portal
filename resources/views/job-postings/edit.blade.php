...
@section('title', 'Edit Lowongan - Job Portal')
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    {{-- Page Title --}}
    <div class="mb-8">
        <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">Edit Lowongan</h1>
        <p class="mt-2 text-sm text-slate-600">Perbarui informasi lowongan <strong>{{ $jobPosting->title }}</strong>.</p>
    </div>

    {{-- Moderation Warning for Active Jobs --}}
    @if($jobPosting->status === 'active')
        <div class="mb-6 p-4 rounded-xl bg-amber-50 border border-amber-200 text-amber-800 text-sm flex items-center gap-3">
            ...
            <div>
                <strong class="font-semibold">Catatan Moderasi:</strong> Lowongan ini saat ini berstatus <strong>Aktif</strong>. Menyimpan perubahan akan mengembalikan status lowongan ke <strong>Menunggu (Pending)</strong> untuk ditinjau kembali oleh admin.
            </div>
        </div>
    @endif
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 sm:p-10">
        <form action="{{ route('job-postings.update', $jobPosting) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="employment_type"
                                ...
                                <option value="Freelance" {{ old('employment_type', $jobPosting->employment_type) === 'Freelance' ? 'selected' : '' }}>Freelance</option>
            ...
            <div>
                ...
                <div class="max-w-sm">
                    ...
                    <input type="date"
                           name="deadline"
                           id="deadline"
                           value="{{ old('deadline', $jobPosting->deadline ? $jobPosting->deadline->format('Y-m-d') : '') }}"
                           min="{{ date('Y-m-d') }}"
            ...
            <div class="pt-6 border-t border-slate-100 flex flex-col-reverse sm:flex-row items-center justify-end gap-3">
                <a href="{{ route('job-postings.show', $jobPosting) }}" class="w-full sm:w-auto px-6 py-3 rounded-xl border border-slate-300 hover:bg-slate-50 text-slate-700 font-semibold text-sm text-center transition-colors">
                    Batal
                </a>
