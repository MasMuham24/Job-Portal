...
@section('title', 'Buat Profil Perusahaan - Job Portal')
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="mb-6">
        <nav class="flex items-center text-sm font-medium text-slate-500 gap-2">
            ...
            <span class="text-slate-900 font-semibold">Buat Profil Perusahaan</span>
        </nav>
    </div>
        ...
        <form action="{{ route('company.store') }}" method="POST" class="space-y-8">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {{-- Company Name (Full width or Col 2) --}}
                    ...
                    <div>
                        ...
                        <span class="block text-[11px] text-slate-400 mt-1">Masukkan URL langsung ke gambar logo perusahaan (opsional).</span>
                        @error('logo')
                            <p class="mt-1.5 text-xs text-red-600 flex items-center gap-1 font-medium">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                <span>{{ $message }}</span>
            ...
            <div class="pt-6 border-t border-slate-100 flex flex-col-reverse sm:flex-row items-center justify-end gap-3">
                <a href="{{ route('dashboard') }}" class="w-full sm:w-auto px-6 py-3 rounded-xl border border-slate-300 hover:bg-slate-50 text-slate-700 font-semibold text-sm text-center transition-colors">
                    Batal
                </a>
